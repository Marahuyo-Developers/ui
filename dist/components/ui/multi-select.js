...
export const MultiSelect = React.forwardRef(({ options, onValueChange, variant, defaultValue = [], placeholder = 'Select options', animation = 0, maxCount = 3, modalPopover = false, asChild = false, className, ...props }, ref) => {
    ...
    return (_jsxs(Popover, { open: isPopoverOpen, onOpenChange: setIsPopoverOpen, modal: modalPopover, children: [_jsx(PopoverTrigger, { asChild: true, children: _jsx(Button, { ref: ref, ...props, onClick: handleTogglePopover, className: cn("border-input data-[placeholder]:text-muted-foreground [&_svg:not([class*='text-'])]:text-muted-foreground focus-visible:border-ring focus-visible:ring-ring/50 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive dark:bg-input/30 dark:hover:bg-input/50 flex items-center justify-between gap-2 rounded-md border bg-transparent px-3 py-2 text-sm whitespace-nowrap shadow-xs transition-[color,box-shadow] outline-none focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50 data-[size=default]:h-9 data-[size=sm]:h-8 *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-2 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 w-full", className), children: selectedValues.length > 0 ? (_jsxs("div", { className: "flex justify-between items-center w-full", children: [_jsxs("div", { className: "flex flex-wrap items-center", children: [selectedValues.slice(0, maxCount).map((value) => {
                                        const option = options.find((o) => o.value === value);
                                        const IconComponent = option?.icon;
                                        return (_jsxs(Badge, { className: cn(multiSelectVariants({ variant })), children: [IconComponent && (_jsx(IconComponent, { className: "h-4 w-4 mr-2" })), option?.label] }, value));
                                    }), selectedValues.length > maxCount && (_jsx(Badge, { className: cn('bg-transparent text-foreground border-foreground/1 hover:bg-transparent', multiSelectVariants({ variant })), children: `+ ${selectedValues.length - maxCount} more` }))] }), _jsxs("div", { className: "flex items-center justify-between gap-2.5", children: [_jsx(Separator, { orientation: "vertical", className: "flex min-h-6 h-full" }), _jsx(ChevronDown, { className: "h-4 cursor-pointer text-muted-foreground" })] })] })) : (_jsxs("div", { className: "flex items-center justify-between w-full ", children: [_jsx("span", { className: "text-sm text-muted-foreground", children: placeholder }), _jsx(ChevronDown, { className: "h-4 cursor-pointer text-muted-foreground" })] })) }) }), _jsx(PopoverContent, { className: "w-auto p-0", align: "start", onEscapeKeyDown: () => setIsPopoverOpen(false), children: _jsxs(Command, { children: [_jsx(CommandInput, { placeholder: "Search...", onKeyDown: handleInputKeyDown }), _jsxs(CommandList, { children: [_jsx(CommandEmpty, { children: "No results found." }), _jsxs(CommandGroup, { children: [_jsxs(CommandItem, { onSelect: toggleAll, className: "cursor-pointer", children: [_jsx("div", { className: cn('mr-2 flex h-4 w-4 items-center justify-center rounded-sm border border-primary', selectedValues.length === options.length
