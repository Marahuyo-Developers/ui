...
function Sortable(props) {
    ...
    return (_jsx(SortableRootContext.Provider, { value: contextValue, children: _jsx(DndContext, { collisionDetection: collisionDetection ?? config.collisionDetection, modifiers: modifiers ?? config.modifiers, sensors: sensors, ...sortableProps, id: id, onDragStart: composeEventHandlers(sortableProps.onDragStart, ({ active }) => setActiveId(active.id)), onDragEnd: composeEventHandlers(sortableProps.onDragEnd, onDragEnd), onDragCancel: composeEventHandlers(sortableProps.onDragCancel, () => setActiveId(null)), accessibility: {
                announcements,
...
export { Root, Content, Item, ItemHandle, Overlay, 
//
