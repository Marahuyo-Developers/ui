...
export function DataTableFilterMenu<TData>({
  ...
  filtersKey = 'filters',
  openMenuShortcut = 'f',
  removeFilterShortcuts = ['backspace', 'delete'],
  ...props
  ...
  const onInputKeyDown = React.useCallback(
    (event: React.KeyboardEvent<HTMLInputElement>) => {
      if (
        removeFilterShortcuts.includes(event.key.toLowerCase()) &&
        !inputValue &&
        selectedColumn
      ) {
  ...
  React.useEffect(() => {
    function onKeyDown(event: KeyboardEvent) {
      ...
      if (
        event.key.toLowerCase() === openMenuShortcut &&
        !event.ctrlKey &&
        !event.metaKey &&
        !event.shiftKey
      ) {
    ...
    <div className="flex flex-wrap items-center gap-2">
      {filters.map((filter) => (
        <DataTableFilterItem
          key={filter.filterId}
          filter={filter}
          removeFilterShortcuts={removeFilterShortcuts}
